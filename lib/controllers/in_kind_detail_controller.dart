...
class InKindDetailController extends GetxController {
  ...
  Future<void> loadGroupDetails() async {
    try {
      print('InKindDetailController.loadGroupDetails - Loading group history for ${group.id}');
      final response = await _groupService.getGroupHistory(group.id);

      if (response.success && response.data != null) {
        final groupData = response.data!;
        print('InKindDetailController.loadGroupDetails - Success, updating groupHistoryData');
        ...
        if (membersData != null) {
          print('InKindDetailController.loadGroupDetails - Found ${membersData.length} members with payment status');
        ...
        if (paymentsData != null) {
          print('InKindDetailController.loadGroupDetails - Found ${paymentsData.length} payments');
        ...
        if (winnersData != null) {
          print('InKindDetailController.loadGroupDetails - Found ${winnersData.length} winners');
        ...
        print('InKindDetailController.loadGroupDetails - Data loaded successfully');
        print('InKindDetailController.loadGroupDetails - current_round: ${groupData['current_round']}');
        print('InKindDetailController.loadGroupDetails - rounds_remaining: ${groupData['rounds_remaining']}');
      } else {
        print('InKindDetailController.loadGroupDetails - Failed: ${response.message}');
      }
    } catch (e) {
      print('InKindDetailController.loadGroupDetails - Error: $e');
    ...
    try {
      ...
      if (response.success && response.data != null) {
        ...
        final String message =
            'Join our In-Kind Ekub group! ${group.name}. Click the link to join: $inviteLink';

        Share.share(
          message,
          subject: 'Invite to join ${group.name} In-Kind Ekub Group',
        );
      } else {
